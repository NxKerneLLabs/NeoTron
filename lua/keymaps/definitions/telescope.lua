-- Caminho Sugerido: lua/keymaps/definitions/telescope.lua
-- (Anteriormente: nvim/lua/keymaps/which-key/telescope.lua)
...
function M.get_mappings(icons, logger)
  ...
  local lsp_icons_map = (icons and icons.lsp) or {} -- Para Telescope LSP finders
  ...
  local function get_ts_fn(name, default_desc)
    if telescope_fns and telescope_fns[name] then
      ...
      return function() logger.warn("[Defs/Telescope] Ação para '"..default_desc.."' não disponível (functions.telescope."..name.." faltando).") end
    end
  end
  ...
  local mappings = {
    ["<leader>f"] = {
      -- name = (ui_icons.search or "") .. " Find/Telescope", -- Nome do grupo é definido em plugins/which-key.lua
      f = { get_ts_fn("find_files", "Find Files"), desc = (ui_icons.file or "") .. " Find Files" },
      r = { get_ts_fn("recent_files", "Recent Files"), desc = (ui_icons.file or "") .. " Recent Files" },
      g = { get_ts_fn("live_grep", "Live Grep"), desc = (misc_icons.grep or "󱎸") .. " Live Grep" },
      b = { get_ts_fn("buffers", "Open Buffers"), desc = (misc_icons.buffer or "󰓩") .. " Open Buffers" },
      m = { get_ts_fn("marks", "Marks"), desc = (misc_icons.mark or "󰃀") .. " Marks" },
      h = { get_ts_fn("help_tags", "Help Tags"), desc = (misc_icons.help or "") .. " Help Tags" },
      ...
      p = { get_ts_fn("projects", "Projects"), desc = (misc_icons.project or "") .. " Projects" },
      s = { get_ts_fn("current_buffer_fuzzy_find", "Search in Buffer"), desc = (ui_icons.fuzzy or "󰍉") .. " Search in Buffer" },
      t = { get_ts_fn("treesitter", "Treesitter Symbols"), desc = (misc_icons.tree or "") .. " Treesitter Symbols" },
    },
    ["<leader>l"] = { -- Mapeamentos Telescope para LSP, sob o grupo <leader>l
      -- name = (ui_icons.code or "󰘧") .. " LSP (Telescope Finders)", -- Nome do grupo <leader>l é definido em plugins/which-key.lua
      -- Estes são adicionados ao grupo <leader>l já existente.
      ...
      -- Se `definitions/lsp.lua` já tem <leader>ls, este vai sobrescrever ou causar comportamento inesperado.
      -- Considere prefixos mais específicos como <leader>lf (find lsp) ou <leader>ls (search lsp)
      s = { get_ts_fn("document_symbols", "Document Symbols"), desc = (misc_icons.symbol or "󰯻") .. " Document Symbols (TS)" },
      S = { get_ts_fn("workspace_symbols", "Workspace Symbols"), desc = (misc_icons.symbol or "󰯻") .. " Workspace Symbols (TS)" },
      -- Exemplo de como adicionar mais pickers do Telescope relacionados ao LSP:
      -- r = { get_ts_fn("lsp_references", "LSP References (TS)"), desc = (lsp_icons_map.References or "󰌷") .. " LSP References (TS)" },
      -- i = { get_ts_fn("lsp_implementations", "LSP Implementations (TS)"), desc = (lsp_icons_map.Implementation or "IMP") .. " LSP Implementations (TS)" },
      -- d = { get_ts_fn("lsp_definitions", "LSP Definitions (TS)"), desc = (lsp_icons_map.Definition or "") .. " LSP Definitions (TS)" },
    },
  }
...
-- Funções utilitárias como reload e list_mappings podem permanecer se você as chama de outros lugares.
-- Elas não são usadas pelo orquestrador de keymaps.
-- Se adaptadas, devem usar o logger passado como parâmetro.

...
return {
  {
    ...
    dependencies = {
      ...
      "zbirenbaum/copilot-cmp",
      {
        ...
        config = function()
          local logger
          local dbg_ok, dbg = pcall(require, "core.debug")
          ...
          if not ls_ok or not luasnip then
            logger.error("Failed to load 'luasnip'. Snippet setup aborted.")
            return
          end
    ...
    config = function()
      ...
      local cp_ok, copilot_cmp = pcall(require, "copilot_cmp")
      if not cp_ok then logger.warn("'copilot_cmp' not found: "..tostring(copilot_cmp)) end
